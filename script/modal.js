  ...
  $(document).on("click keypress", ".modal_button", function () {
    ...
    fetch("https://anjungme.github.io/project_cineflex/json/goods.json")
      ...
      .then((json) => {
        ...
        var { title, type, age_grade, src, m_src, h_src, duration, description, Ep, age_grade, ContentIntroduction } = modalMovies;
        ...
        var modalMoviesType = movies.filter((movie) => movie.type === type && movie.title !== title); //추가 해당되는 컨텐츠는 빼버림
        modalMoviesType = shuffleArray(modalMoviesType);
        $("#rel_contents").html("");
        $("#rel_contents").append(`<p><span>${title} 와(과) 관련된 콘텐츠</span></p>`);
        for (var i = 0; 6 > i; i++) {
